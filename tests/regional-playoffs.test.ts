...
describe('regional split playoffs', () => {
  ...
  test('Upper Round 1 and Upper Round 2 losers meet in Middle Round 1', () => {
    ...
    ;(['Americas', 'EMEA', 'Pacific', 'China'] as const).forEach((region) => {
      ...
      const expected = new Set(
        [...upperOne, ...upperTwo].map((fixture) =>
          fixture.winnerId === fixture.aId ? fixture.bId : fixture.aId,
        ),
      )
      expect(middleOne).toHaveLength(4)
      expect(new Set(middleOne.flatMap((fixture) => [fixture.aId, fixture.bId]))).toEqual(expected)
    })
  })
  test('Kickoff uses BO5 only for the three bracket finals', () => {
    const state = advance(6),
      finals = ['Upper Final', 'Middle Final', 'Lower Final']
    ...
    expect(kickoff).toHaveLength(120)
    expect(
      state.fixtures.filter((fixture) => fixture.phase === 'Kickoff' && !fixture.bId),
    ).toHaveLength(0)
    expect(
      kickoff.every((fixture) =>
        finals.includes(fixture.label) ? fixture.bestOf === 5 : fixture.bestOf === 3,
      ),
    ).toBeTrue()
  })
  test('Kickoff Round 2 pairs each Round 1 winner with a bye team', () => {
    ...
    const byes = Object.values(state.teams)
      .filter((team) => team.region === region && state.kickoff[team.id].openingBye)
      .map((team) => team.id)
    const roundOne = state.fixtures.filter(
      (fixture) =>
    ...
    const upperTwo = state.fixtures.filter(
      (fixture) =>
        fixture.phase === 'Kickoff' &&
        fixture.region === region &&
        fixture.label === 'Upper Round 2',
    )
    expect(upperTwo).toHaveLength(4)
    expect(upperTwo.every((fixture) => byes.includes(fixture.aId))).toBeTrue()
    expect(upperTwo.map((fixture) => fixture.bId).sort()).toEqual(
      roundOne.map((fixture) => fixture.winnerId).sort(),
    )
  })
  ...
  test('Stage 1 playoff results produce three Masters 2 qualifiers per region', () => {
    const seeded = advance(15),
      state = advance(18)
    const playoffs = state.fixtures.filter(
      (fixture) => fixture.phase === 'Stage 1' && fixture.stage === 'Playoffs',
    )
    ...
    ;(['Americas', 'EMEA', 'Pacific', 'China'] as const).forEach((region) => {
      ...
      const lowerOne = playoffs.filter(
        (fixture) => fixture.region === region && fixture.label === 'Lower Round 1',
      )
      expect(upperQf).toHaveLength(2)
      expect(upperQf.flatMap((fixture) => [fixture.aId, fixture.bId]).sort()).toEqual(
        seeds.slice(2, 6).sort(),
      )
  ...
  test('Stage 1 qualifiers are used to seed Masters 2', () => {
    ...
    const opening = state.fixtures.filter(
      (fixture) => fixture.phase === 'Masters 2' && fixture.label === 'Swiss Opening',
    )
  ...
  test('Stage 2 playoff results produce four Champions qualifiers per region', () => {
    ...
    const playoffs = state.fixtures.filter(
      (fixture) => fixture.phase === 'Stage 2' && fixture.stage === 'Playoffs',
    )
    expect(playoffs).toHaveLength(48)
    expect(playoffs.filter((fixture) => fixture.status === 'completed')).toHaveLength(48)
    ;(['Americas', 'EMEA', 'Pacific', 'China'] as const).forEach((region) => {
    ...
    ;(['Americas', 'EMEA', 'Pacific', 'China'] as const).forEach((region) => {
      ...
      expect(
        Object.values(state.teams).filter(
          (team) => team.region === region && state.kickoff[team.id].status === 'active',
        ),
      ).toHaveLength(0)
    })
  })
