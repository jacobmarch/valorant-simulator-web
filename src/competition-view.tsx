...
function FixtureCard({s,fixture,featured=false,onOpenMatch}:{s:GameState;fixture:Fixture;featured?:boolean;onOpenMatch?:(matchId:string)=>void}){
  ...
  return <article onClick={result&&onOpenMatch?()=>onOpenMatch(result.id):undefined} role={result&&onOpenMatch?"button":undefined} onKeyDown={result&&onOpenMatch?event=>{if(event.key==="Enter"||event.key===" "){event.preventDefault();onOpenMatch(result.id)}}:undefined} tabIndex={result&&onOpenMatch?0:undefined} className={`matchup-card ${fixture.status} ${managed?'managed':''} ${featured?'featured':''}`}><header><span>{fixture.label}</span><span>{fixture.status==='completed'?'Final':`Week ${fixture.week} · BO${fixture.bestOf}`}</span></header>
...
function Schedule({s,fixtures,title,empty,onOpenMatch}:{s:GameState;fixtures:Fixture[];title:string;empty?:string;onOpenMatch?:(matchId:string)=>void}){return <section className="panel schedule-panel"><PanelTitle eyebrow="MATCH DESK" title={title} right={<Badge>{fixtures.length} matches</Badge>}/>{fixtures.length?<div className="matchup-list">{fixtures.map(fixture=><FixtureCard s={s} fixture={fixture} featured onOpenMatch={onOpenMatch} key={fixture.id}/>)}</div>:<div className="competition-empty"><strong>Nothing on the slate</strong><span>{empty??'No matches are scheduled in this window.'}</span></div>}</section>}
...
function KickoffView({s,region,week,setWeek,onOpenMatch}:{s:GameState;region:Region;week:number;setWeek:(week:number)=>void;onOpenMatch?:(matchId:string)=>void}){
  const ids=Object.values(s.teams).filter(team=>team.region===region).map(team=>team.id)
  ...
  return <><section className="panel format-explainer"><div><div className="eyebrow">HOW THE FORMAT WORKS</div><h2>Three lives. Seeds with an opening bye.</h2><p>Eight teams play in round one. The four organizations that reached last year's Champions start in the second upper-bracket round. Losses then move teams through the middle and lower paths.</p></div><div className="life-key"><span>Upper</span><b>→</b><span>Middle</span><b>→</b><span>Lower</span><b>→</b><span>Out</span></div></section>
    ...
    <div className="competition-split"><section className="panel"><PanelTitle eyebrow={`${region.toUpperCase()} QUALIFICATION RACE`} title="Kickoff standings" right={<Badge color={colors[region]}>3 qualify</Badge>}/><TeamTable s={s} region={region} phase="Kickoff"/></section><div><Matchday week={week} setWeek={setWeek} min={1} max={6}/><Schedule onOpenMatch={onOpenMatch} s={s} fixtures={fixtures} title={`Week ${week} matchups`}/></div></div></>
}
function LeagueView({s,phase,region,week,setWeek,stage,setStage,onOpenMatch}:{s:GameState;phase:'Stage 1'|'Stage 2';region:Region;week:number;setWeek:(week:number)=>void;stage:'opening'|'playoffs';setStage:(stage:'opening'|'playoffs')=>void;onOpenMatch?:(matchId:string)=>void}){
  ...
  return <>
    ...
    {stage==='opening'
      ?<div className="competition-split"><section className="panel"><PanelTitle eyebrow={region.toUpperCase()+' / '+phase.toUpperCase()} title="League table" right={<Badge color={colors[region]}>{"Top 8 playoffs · "+qualificationCount+" qualify"}</Badge>}/><TeamTable s={s} region={region} phase={phase}/></section><div><Matchday week={week} setWeek={setWeek} min={info.start} max={info.end}/><Schedule onOpenMatch={onOpenMatch} s={s} fixtures={current} title={region+' league fixtures'} empty="The league schedule begins when this split starts."/></div></div>
      :<><PlayoffBracket onOpenMatch={onOpenMatch} s={s} fixtures={playoffFixtures}/><Matchday week={week} setWeek={setWeek} min={info.start} max={info.end}/><Schedule onOpenMatch={onOpenMatch} s={s} fixtures={current} title={region+' playoff matchups'} empty="The regional playoff bracket fills after league play concludes."/></>}
  </>
}
...
function PlayoffBracket({s,fixtures,onOpenMatch}:{s:GameState;fixtures:Fixture[];onOpenMatch?:(matchId:string)=>void}){
  const playoff=fixtures.filter(f=>stageFor(f)==='Playoffs'),final=playoff.find(f=>f.label==='Grand Final')
  const lane=(name:'upper'|'lower')=><section className={`bracket-lane ${name}`}><header><div><b>{name==='upper'?'Upper bracket':'Lower bracket'}</b><span>{name==='upper'?'A loss moves a team to the lower path':'Every series is elimination'}</span></div></header><div className="bracket-path">{bracketColumns.filter(c=>c.lane===name).map(column=>{const matches=playoff.filter(f=>f.label===column.key);return <div className="bracket-step" key={column.key}><h3>{column.title}<small>{matches.length||'—'} matches</small></h3>{matches.length?matches.map(f=><FixtureCard s={s} fixture={f} onOpenMatch={onOpenMatch} key={f.id}/>):<div className="bracket-placeholder"><strong>Awaiting teams</strong><span>Previous results fill this round.</span></div>}</div>})}</div></section>
  return <section className="playoff-board"><div className="bracket-legend"><span><i/>Upper path</span><span><i/>Elimination path</span></div>{lane('upper')}{lane('lower')}<div className="grand-final"><div><div className="eyebrow">TITLE MATCH</div><h2>Grand final</h2><p>The upper-bracket winner meets the survivor of the lower bracket.</p></div>{final?<FixtureCard s={s} fixture={final} featured onOpenMatch={onOpenMatch}/>:<div className="bracket-placeholder"><strong>Finalists not decided</strong><span>The title match appears when both paths finish.</span></div>}</div></section>
}
...
function InternationalView({s,phase,stage,setStage,onOpenMatch}:{s:GameState;phase:'Masters 1'|'Masters 2'|'Champions';stage:'opening'|'playoffs';setStage:(stage:'opening'|'playoffs')=>void;onOpenMatch?:(matchId:string)=>void}){
  ...
  return <><div className="stage-tabs"><button className={stage==='opening'?'active':''} onClick={()=>setStage('opening')}><small>01</small>{phase==='Champions'?'Group stage':'Swiss stage'}</button><button className={stage==='playoffs'?'active':''} onClick={()=>setStage('playoffs')}><small>02</small>Playoff bracket</button></div>{stage==='opening'?(phase==='Champions'?<ChampionsGroups s={s} fixtures={fixtures.filter(f=>stageFor(f)==='Groups')}/>:<SwissView s={s} phase={phase} fixtures={fixtures}/>):<PlayoffBracket onOpenMatch={onOpenMatch} s={s} fixtures={fixtures}/>}<Schedule onOpenMatch={onOpenMatch} s={s} fixtures={current} title={`${phase} · current matchups`} empty={s.week<events[phase].start?'The qualified field appears when the event begins.':'No matches remain this week.'}/></>
}
export function CompetitionV2({s,onOpenMatch}:{s:GameState;onOpenMatch?:(matchId:string)=>void}){
  ...
  return <div className="page competition-hub"><div className="page-head"><div><div className="eyebrow">COMPETITION CENTER / WEEK {s.week}</div><h1>The road through {s.season}.</h1><p>Move event by event through the season. Every format has its own standings, rules, and match path.</p></div></div>
    ...
    {phase==='Kickoff'?<KickoffView s={s} region={region} week={week} setWeek={setWeek} onOpenMatch={onOpenMatch}/>:phase==='Stage 1'||phase==='Stage 2'?<LeagueView s={s} phase={phase} region={region} week={week} setWeek={setWeek} stage={stage} setStage={setStage} onOpenMatch={onOpenMatch}/>:<InternationalView s={s} phase={phase} stage={stage} setStage={setStage} onOpenMatch={onOpenMatch}/>}
  </div>
}
